...
static int mchp_coreqspi_set_operate_mode(struct mchp_coreqspi *qspi, const struct spi_mem_op *op)
{
...
static int mchp_coreqspi_exec_op(struct spi_slave *slave, const struct spi_mem_op *op)
{
       struct mchp_coreqspi *qspi = dev_get_priv(slave->dev->parent);
...
static bool mchp_coreqspi_supports_op(struct spi_slave *slave, const struct spi_mem_op *op)
{
...
static int mchp_coreqspi_adjust_op_size(struct spi_slave *slave, struct spi_mem_op *op)
{
       if (op->data.dir == SPI_MEM_DATA_OUT) {
